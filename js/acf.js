...
acf.fn.iterate = function(callback, time) {
	...
	function setCoef(params){
		...
		if ( _("body").hasClass('landscapeMode') ) {
			...
			if(params.x instanceof Array || params.y instanceof Array) {
					if(params.x)params.x = params.x.length == 2 ? params.x[0] : params.x[0];
					if(params.y)params.y = params.y.length == 2 ? params.y[0] : params.y[0];
				} else {
			...
			if(params.width === undefined && params.height === undefined && params.x === undefined && params.y === undefined) {
				...
				if(params.x instanceof Array || params.y instanceof Array) {
					if(params.x)params.x = params.x.length == 2 ? params.x[1] : params.x[0];
					if(params.y)params.y = params.y.length == 2 ? params.y[1] : params.y[0];
				} else {
					params.x = params.x === undefined ? undefined : params.x;
					params.y = params.y === undefined ? undefined : params.y;
				}
...
acf.fn.delay = function(fn,time) {
	var that = this;
	var real_time = time/this.length;
	setTimeout(fn,time);
};

acf.fn.Delay = function(prop, value, time){
	for(var el = 0; el < this.length; el++) {
		var self = this[el];
